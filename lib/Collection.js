...
Object
  ...
  .forEach(function (name) {
    ...
    Collection.prototype[method] = function (name, properties) {
      ...
      if (metric) {
        assert(metric.constructor.name === MetricConstructor.name,
          `trying to use metric ${name} as ${MetricConstructor.name} but it's ${metric.constructor.name}`);
        return metric;
      }

      metric = new MetricConstructor(properties);
